...
RLAPI void rlSetVertexAttributeDefault(
    ...
    int count);  // Set vertex attribute default value, when attribute to
                 // provided
...
RLAPI Matrix
rlGetMatrixProjectionStereo(int eye);  // Get internal projection matrix for
...
#define GL_GLEXT_PROTOTYPES
                                  ...
                                  // library -> not required, platform layer
    ...
    for (int i = 0; i < 6 * mipmapCount; i++) {
      ...
      if (data == NULL) {
        if (format < RL_PIXELFORMAT_COMPRESSED_DXT1_RGB) {
          ...
          TRACELOG(RL_LOG_WARNING,
                   "TEXTURES: Empty cubemap creation does not support "
                   "compressed format");
      } else {
        if (format < RL_PIXELFORMAT_COMPRESSED_DXT1_RGB)
          glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + face, mipmapLevel,
    ...
    case RL_ATTACHMENT_COLOR_CHANNEL7: {
      if (texType == RL_ATTACHMENT_TEXTURE2D)
        glFramebufferTexture2D(GL_FRAMEBUFFER,
                               GL_COLOR_ATTACHMENT0 + attachType, GL_TEXTURE_2D,
                               texId, mipLevel);
